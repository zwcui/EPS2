		...
		<div class='panel panel-default grid'>
			<div id="listtoolbar" class="panel-body btn-group">
				<button type="button" class="btn btn-custom-default"
					...
					<i class="icon-plus-sign purple">新增用户</i>
				</button>
				<button id="viewUserRoleButton" type="button" class="btn btn-custom-default"
					onclick="viewUserRole()" data-toggle="modal">
					<i class="icon-pencil blue">用户岗位</i>
				</button>
<!-- 				<button type="button" class="btn btn-custom-default" -->
<!-- 					onclick="LockUser()"> -->
<!-- 					<i class="icon-ok green">锁定用户</i> -->
<!-- 				</button> -->
				<button type="button" class="btn btn-custom-default"
					onclick="deleteUser()">
					<i class="icon-trash red">删除用户</i>
				</button>
			</div>
	...
	<script>
	var b = new Base64(); 
	var sourceDataUrl = "${pageContext.request.contextPath}/ShowAllUser";
	var grid_data;
...
// 							multiselect : true,
		...
		function viewUserRole() {
			...
			if (selectRow.length < 1
				...
				alert("当前新增用户未保存，无法查看用户岗位！");
				return;
			} else {
				var sPageTitle = "<%=java.net.URLEncoder.encode(java.net.URLEncoder.encode("用户管理","UTF-8"),"UTF-8")%>";
				var sUserID = b.encode(selectRow.userID);
				var openUrl = "../OrgManage/userroles.jsp?UserID="+sUserID+"&ParentPageTitle="+sPageTitle;
