		...
		<div class='panel panel-default grid'>
			<div id="listtoolbar" class="panel-body btn-group">
				<button type="button" class="btn btn-custom-default"
					...
					<i class="icon-plus-sign purple">新增机构</i>
				</button>
<!-- 				<button id="viewOrgButton" type="button" class="btn btn-custom-default" -->
<!-- 					onclick="viewOrgRole()" data-toggle="modal"> -->
<!-- 					<i class="icon-pencil blue">机构岗位</i> -->
<!-- 				</button> -->
<!-- 				<button type="button" class="btn btn-custom-default" -->
<!-- 					onclick="LockOrg()"> -->
<!-- 					<i class="icon-ok green">锁定机构</i> -->
<!-- 				</button> -->
				<button type="button" class="btn btn-custom-default"
					onclick="deleteOrg()">
					<i class="icon-trash red">删除机构</i>
				</button>
			</div>
	...
	<script>
	var sourceDataUrl = "${pageContext.request.contextPath}/ShowAllOrg";
	var grid_data;
...
// 							multiselect : true,
		...
		function saveRecord(id) {
			...
			var url = '${pageContext.request.contextPath}/UpdateOrg';
			if (typeof (record.serialNo) == "undefined"
					|| record.serialNo.length == 0) {
				url = '${pageContext.request.contextPath}/CreateOrg';
			}
			var data = {
		...
		function viewOrgRole() {
			...
			if (selectRow.length < 1
					|| typeof (selectRow.orgID) == "undefined"
					|| (selectRow.orgID).length == 0) {
				alert("当前新增机构未保存，无法查看机构下岗位！");
				return;
			} else {
				// 				$('#viewOrderInfo').modal('toggle');
				// 				$('#viewOrderInfo').modal('show');
				$("#modal").modal({
					remote : "../OrgManage/orgroles.jsp",
									backdrop : 'static',
									keyboard : false
				});
		...
		function deleteOrg() {
			...
			if (selectRow.length < 1
				...
				$
						.ajax({
							url : '${pageContext.request.contextPath}/DeleteOrg',
							dataType : 'json',
							type : 'POST',
							data : data,
							success : function(response) {
...
// 			$("#orglist").setGridWidth($(window).width() * 0.96);
// 			$('.modal').each(
// 					function(i) {
