...
<body class='main page' >
	...
	<div class="page-content" id="pagecontent">
		...
		<div id="listtoolbar" class="panel-body btn-group">
				...
				<table id="userrolelist" class="table table-striped table-bordered table-hover no-margin-bottom no-border-top"></table>
				<div id="userrolelist-pager"></div>
	...
	<script>
	var b = new Base64(); 
	var userrolelistDataUrl = "${pageContext.request.contextPath}/ShowUserRoleByUserID";
	var userrolelist_data;
	var sUserID = "";
	function initUserRoleList() {
		var data = null;
		data = {
			userID : sUserID,
			modelName:'RoleList',
		...
		$
				.ajax({
					url : userrolelistDataUrl,
					dataType : 'json',
					type : 'POST',
					data : data,
					success : function(response) {
						var userrolelist_data = response.data;
						var colnames = response.colNames;
						var colmodel = modelPreTreat(response.column);
						var lastSel;
						var grid_selector = "#userrolelist";
						var pager_selector = "#userrolelist-pager";
						jQuery(grid_selector).jqGrid({
// 							direction: "rtl",
							data: userrolelist_data,
							datatype: "local",
							height: 'auto',
							colNames:colnames,
							colModel:colmodel,
...
// 								showQuery: true
	...
	function refreshUserRoleList(grid_selector) {
		$.ajax({
			url : userrolelistDataUrl,
			dataType : 'json',
			type : 'POST',
	...
	function createNewUserRole() {
		...
		var openUrl = "../OrgManage/selectrolelist.jsp?UserID="+sUserID;
		window.open(openUrl,"newUserRole","modal=yes,,scrollbars=noheight="+iHeight+", width="+iWidth+", top="+iTop+", left="+iLeft); 
	}
	<!--新增用户岗位 - end-->
	...
	function saveRecord(id) {
		...
		$.ajax({
			...
			success : function(response) {
				...
				alert(result);
				refreshUserRoleList(grid_selector);
			},
			error:function(xhr,errorText,errorType){
				alert("保存失败！");
			},
	...
	function deleteUserRole() {
		...
		if (selectRow.length < 1
				|| typeof (selectRow.userID) == "undefined"
				|| (selectRow.userID).length == 0) {
			refreshUserRoleList(grid_selector);
		} else {
			var data = null;
			data = {
