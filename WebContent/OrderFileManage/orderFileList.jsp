...
<%
	...
	String CurOrgID = (String)s.getAttribute("orgID");
%>
<head>
...
<body class='main page' >
	...
	<div class="page-content" id="pagecontent">
		<div class='panel panel-default grid'>
			<div id="listtoolbar" class="panel-body btn-group">
				<form id="orderFileUploadForm" class="form-inline"  role="form" enctype="multipart/form-data" action="" method="post"  >
					<% if(!"ReadOnly".equals(sRightType)){%>
					<div class="form-group">
						<label class="sr-only" for="inputfile">文件输入</label> 
						<input type="file" id="inputfile" name="inputfile">
					</div>
					<button type="button" class="btn btn-custom-default btn-sm"
						onclick="doUploadFile('#orderFileUploadForm')">
						<i class="icon-upload green">上传</i>
					</button>
					<%} %>
					<button type="button" class="btn btn-custom-default btn-sm"
						onclick="DownLoadFile()">
						<i class="icon-download purple">下载</i>
					</button>
					<% if(!"ReadOnly".equals(sRightType) && !("Flow".equals(sRightType) && !"0001".equals(CurOrgID))){%>
					<button type="button" class="btn btn-custom-default btn-sm "
						onclick="deleteOrderFile()">
						<i class="icon-trash red">删除</i>
					</button>
					<%} %>
					<div>
						<label class="sr-only" for="orderSerialNo">订单编号</label>
						<input type="hidden" id="orderSerialNo" name="orderSerialNo"  />
					</div>
				</form>
				</div>
				<table id="orderfilelist" class="table table-striped table-bordered table-hover no-margin-bottom no-border-top"></table>
				<div id="orderfilelist-pager"></div>
	...
	<script>
	var b = new Base64(); 
	var orderfilelistDataUrl = "${pageContext.request.contextPath}/ShowOrderFileList";
	var orderfilelist_data;
	var sOrderSerialNo = "";
	var sIsInEdit = false;
	var sRightType = "";

	<!--上传文件 - start-->
	function doUploadFile(form_selector){
		var grid_selector = "#orderfilelist";
		var filePath = $("#inputfile").val();
		var orderSerialNo = $("#orderSerialNo").val();
		if(typeof(orderSerialNo) == "undefined" || orderSerialNo.length == 0){
			alert("未成功加载订单编号，无法上传订单附件");
			return false;
		}
		...
		var options = {
		   url:'${pageContext.request.contextPath}/UploadFileServlet',
		   success:function(response){
				...
				alert(result);
				refreshOrderFileList(grid_selector);
				return false;
			},
	...
	function DownLoadFile(){
		...
		if (selectRow.length < 1
			...
			form.attr("action","${pageContext.request.contextPath}/DownloadFileServlet");
			var input1=$("<input>");
			input1.attr("type","hidden");
			input1.attr("name","serialNo");
	...
	function deleteOrderFile(){
		...
		if (selectRow.length < 1
			...
			$
					.ajax({
						url : '${pageContext.request.contextPath}/DeleteOrderFile',
						dataType : 'json',
						type : 'POST',
						data : data,
						success : function(response) {
	...
	function initOrderFileList() {
		$("#orderSerialNo").val(sOrderSerialNo);
		var data = null;
		data = {
			serialNo : sOrderSerialNo,
			modelName:'DocumentList',
		}
		$
				.ajax({
					url : orderfilelistDataUrl,
					dataType : 'json',
					type : 'POST',
					data : data,
					success : function(response) {
						var orderfilelist_data = response.data;
						var colnames = response.colNames;
						var colmodel = modelPreTreat(response.column);
						var lastSel;
						var grid_selector = "#orderfilelist";
						var pager_selector = "#orderfilelist-pager";
						jQuery(grid_selector).jqGrid({
// 							direction: "rtl",
							data: orderfilelist_data,
							datatype: "local",
							height: 'auto',
							colNames:colnames,
							colModel:colmodel,
...
// 								showQuery: true
		...
		function refreshOrderFileList(grid_selector) {
			sIsInEdit = false;
			$.ajax({
				url : orderfilelistDataUrl,
				dataType : 'json',
